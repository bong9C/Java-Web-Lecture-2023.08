...
-- 책 가격의 내림차순(descending order)으로 정렬,같으면 책 이름의 오름차순(ascending order)으로 정렬
...
select sum(saleprice) as sum_sales, avg(saleprice) as average_sales,
    max(saleprice)max_sales, min(saleprice) min_sales       -- as 생략가능
    from orders;
-- 대한미디어에서 출간한 책 가격의 평균은?
select avg(price) as average_price from book where publisher like '%대한미디어%'; -- as average_price 생략가능(표헤드이름)